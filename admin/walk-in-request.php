<?php
	...
	if(isset($_POST["brgy_clearance"])) { 
		...
		$pdf = new Fpdi();
		$pdf->AddPage();
		$pdf->setSourceFile('../assets/pdf/clearance2.pdf');
		$tplIdx = $pdf->importPage(1);
		$pdf->SetTitle("BARANGAY SAN VICENTE");  
		...
		$pdf->SetXY(48.5, 145.5);
		$pdf->Write(0, $_POST['day']);

		$pdf->SetXY(68.3, 145.5);
		$pdf->Write(0, strtoupper($_POST['month']));

		$pdf->SetXY(102.3, 145.5);
		$pdf->Write(0, "3");

		ob_end_clean();
		$pdf->Output('I', 'BARANGAY SAN VICENTE.pdf');
	}
	...
	if(isset($_POST["residency"])) { 
		...
		$pdf = new Fpdi();
		$pdf->AddPage();
		$pdf->setSourceFile('../assets/pdf/residency2.pdf');
		$tplIdx = $pdf->importPage(1);
		$pdf->SetTitle("BARANGAY SAN VICENTE");  
		$pdf->useTemplate($tplIdx, 0, 0, 200, 290);
		$pdf->SetFont('Times', 'B', 11);
	...
	if(isset($_POST["indigency"])) { 
		...
		$pdf = new Fpdi();
		$pdf->AddPage();
		$pdf->setSourceFile('../assets/pdf/indigency2.pdf');
		$tplIdx = $pdf->importPage(1);
		$pdf->SetTitle("BARANGAY SAN VICENTE");  
		$pdf->useTemplate($tplIdx, 0, 0, 200, 290);
		$pdf->SetFont('Times', 'B', 11);
										...
										<td>
											<center>
												...
												<form method="POST" target="_blank">
													...
													<button type="submit" name="brgy_clearance" class="btn btn-block green radius-xl" style="float: right;">
													PRINT</button><br><br>
												</form>
												<?php	
													}
